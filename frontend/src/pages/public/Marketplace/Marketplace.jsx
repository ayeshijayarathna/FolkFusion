...
import {
  ...
  FiMapPin, FiPackage, FiCreditCard, FiTruck, FiStar,
  FiChevronDown, FiArrowRight, FiEye, FiRefreshCw,
} from 'react-icons/fi';
import { BsBank2, BsCashCoin, BsGrid3X3Gap, BsListUl } from 'react-icons/bs';
...
const SLIDES = [
  {
    image: '/images/canem.jpg',
    tag: 'Artisan Picks',
    title: 'Cane Work\n& Woven Crafts',
    sub: 'Handwoven cane and rattan creations shaped with patience and precision — bringing natural elegance into everyday living.',
    cta: 'View Collection',
    category: 'Cane Work',
  ...
  {
    image: '/images/folkjevel.jpg',
    tag: 'Heritage Adornments',
    title: 'Folk Jewellery\nCollection',
    sub: 'Timeless pieces inspired by tradition — handcrafted jewellery that reflects culture, identity, and artistic heritage in every detail.',
    cta: 'Explore Collection',
    category: 'Folk Jewellery',
    accent: 'var(--color-muted-clay)',
    accentHex: '#C97B5A',
    accentLight: '#FDF6EE',
  },
  {
    ...
    tag: 'Clay & Earth',
    title: 'Pottery & Clay\nCreations',
    sub: "Earth-fired vessels and sculptures from traditional kilns — timeless forms rooted in Sri Lanka's ancient ceramic heritage.",
    cta: 'Discover More',
  ...
  },
  {
    ...
    tag: 'Rare Finds',
    title: 'Traditional\nPuppetry Arts',
    sub: 'Hand-carved and hand-painted string puppets kept alive by dedicated artisan families across the southern provinces.',
    cta: 'Meet the Puppets',
    category: 'Puppetry',
    accent: 'var(--color-muted-clay)',
    accentHex: '#C97B5A',
    accentLight: '#FDF6EE',
...
const Toast = ({ msg, type, onDone }) => {
  ...
  return createPortal(
    <div
      className={`fixed bottom-7 left-1/2 -translate-x-1/2 z-[99999] ${bg} text-[color:var(--color-warm-sand)] px-6 py-3.5 rounded-xl shadow-2xl font-bold text-[13px] mp-fadein flex items-center gap-2.5 max-w-sm whitespace-nowrap`}
      style={BASKERVILLE}
    >
...
const Modal = ({ open, onClose, width = 700, children }) => {
  ...
  return createPortal(
    <div
      onClick={onClose}
      className="fixed inset-0 z-[8000] flex items-center justify-center p-5 bg-gray-800/65 mp-fadein"
    >
      <div
...
// ─── Hero Carousel ────────────────────────────────────────────────────────────
const HeroCarousel = ({ onCategorySelect, onSearch }) => {
  ...
  return (
    ...
    >
        ...
        >
          <div className="absolute top-0 right-0 bottom-0" style={{ left: '38%' }}>
            ...
            <div
              ...
              style={{
                background: `linear-gradient(to right, ${s.accentLight} 0%, ${s.accentLight}dd 8%, ${s.accentLight}88 25%, ${s.accentLight}22 50%, transparent 72%)`,
              }}
            />
          </div>
        </div>
      ...
      <div
        ...
        style={{ paddingLeft: 'clamp(32px, 7vw, 110px)', maxWidth: '50%' }}
      >
        <div key={current} className="mp-fadeup">
          <h1
            className="font-bold m-0 mb-5 leading-[1.1] whitespace-pre-line"
            style={{
              ...CINZEL,
          ...
          <p
            className="m-0 mb-8 leading-[1.8]"
            style={{
              ...BASKERVILLE,
              fontSize: 'clamp(13px, 1.15vw, 15px)',
              color: 'color-mix(in srgb, var(--color-deep-brown) 80%, transparent)',
              maxWidth: 400,
            }}
          >
            {slide.sub}
        ...
        {/* Search */}
        <div
          className="mt-9 flex items-center gap-0 rounded-2xl py-2 pl-4 pr-2 border"
          style={{
            maxWidth: 440,
            background: 'rgba(255,255,255,0.7)',
            backdropFilter: 'blur(12px)',
            borderColor: 'color-mix(in srgb, var(--color-muted-clay) 16%, transparent)',
          }}
        >
          ...
          <button
            ...
            style={{ ...BASKERVILLE, background: slide.accent }}
          >
            Search
          </button>
        </div>
      ...
      {/*dot indicators */}
      <div
        className="absolute bottom-7 z-30 flex items-center gap-2"
        style={{ left: 'clamp(32px, 7vw, 110px)' }}
      >
        {SLIDES.map((_, i) => (
          <button
            key={i}
            onClick={() => go(i)}
            className="rounded-full border-none p-0 cursor-pointer transition-all duration-400"
            style={{
              width: i === current ? 28 : 8,
              height: 8,
              background: i === current ? slide.accent : `color-mix(in srgb, ${slide.accent} 25%, transparent)`,
            }}
          />
        ))}
        <span
          className="ml-3 text-[11px] font-bold tracking-widest text-[color:var(--color-muted-clay)]/50"
          style={BASKERVILLE}
        >
      ...
      {[
        { dir: -1, Icon: FiChevronLeft,  pos: 'right-[72px]' },
        { dir:  1, Icon: FiChevronRight, pos: 'right-6'      },
      ].map(({ dir, Icon, pos }) => (
        <button
          key={dir}
      ...
      <div className="grid grid-cols-2 min-h-[480px]">
        <div className="bg-[color:var(--color-warm-sand)] rounded-tl-[20px] rounded-bl-[20px] overflow-hidden">
          <div className="h-[380px] overflow-hidden">
            <img
              src={imgs[imgIdx]?.url || primaryImg?.url}
        ...
        <div className="p-8 flex flex-col gap-3.5">
          <button
            onClick={onClose}
            className="self-end bg-transparent border-none cursor-pointer text-[color:var(--color-deep-brown)] flex items-center justify-center w-8 h-8 rounded-full hover:bg-[color:var(--color-warm-sand)] transition-colors"
          >
            <FiX size={20}/>
          </button>
          ...
          {item.isFeatured && (
            <span
              className="inline-flex items-center gap-1 bg-[color:var(--color-muted-clay)]/15 text-[color:var(--color-muted-clay)] border border-[color:var(--color-muted-clay)]/30 px-3 py-1 rounded-full text-[11px] font-bold w-fit"
              style={BASKERVILLE}
            >
          ...
          <div>
            <h2 className="m-0 mt-1 mb-1.5 font-bold text-[18px] text-[color:var(--color-deep-brown)] leading-snug" style={CINZEL}>
              {item.listingTitle}
            </h2>
            <p className="m-0 text-xs text-[color:var(--color-muted-clay)] tracking-wide uppercase" style={BASKERVILLE}>
              {item.artwork?.category} · by {item.artist?.fullName}
            </p>
          </div>
          ...
          <div className="flex gap-3 items-center mt-auto">
            <div className="flex items-center border border-[color:var(--color-muted-clay)]/25 rounded-xl overflow-hidden">
              <button
                ...
                className="w-9 h-10 border-none bg-white cursor-pointer flex items-center justify-center text-[color:var(--color-deep-brown)] hover:bg-[color:var(--color-warm-sand)] transition-colors"
              >
                <FiMinus size={14}/>
              </button>
...
const CartDrawer = ({ open, cart, onClose, onUpdate, onRemove, onCheckout }) => {
  ...
  return createPortal(
    <>
      ...
      <div
        className={`fixed top-0 right-0 z-[8600] w-[400px] max-w-full h-screen bg-[color:var(--color-warm-sand)] shadow-[-8px_0_48px_rgba(0,0,0,0.2)] border-l-2 border-[color:var(--color-muted-clay)]/20 flex flex-col transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="px-5 py-5 pb-4 border-b border-[color:var(--color-muted-clay)]/20 flex items-center justify-between bg-[color:var(--color-deep-brown)]">
          <div className="flex items-center gap-2.5">
            ...
            <span className="bg-[color:var(--color-muted-clay)] text-[color:var(--color-warm-sand)] rounded-full px-2.5 py-0.5 text-[11px] font-bold" style={BASKERVILLE}>
              {cart.reduce((s, c) => s + c.qty, 0)}
            </span>
          </div>
          <button onClick={onClose} className="border-none bg-transparent cursor-pointer text-[color:var(--color-warm-sand)]/70 flex items-center hover:text-[color:var(--color-warm-sand)] transition-colors">
            <FiX size={20}/>
          </button>
        </div>
            ...
            return (
              <div key={ci._id} className="flex gap-3 mb-3.5 pb-3.5 border-b border-[color:var(--color-muted-clay)]/10">
                <div className="w-16 h-16 rounded-xl overflow-hidden bg-[color:var(--color-deep-brown)]/10 flex-shrink-0">
                  {img
                    ? <img src={img.url} alt={ci.listingTitle} className="w-full h-full object-cover" />
                    : <div className="h-full flex items-center justify-center text-[color:var(--color-deep-brown)]"><FiPackage size={22}/></div>
                  }
                </div>
                ...
                <button
                  onClick={() => onRemove(ci._id)}
                  className="self-start bg-transparent border-none cursor-pointer text-red-500 flex items-center p-1 rounded-md hover:bg-red-50 transition-colors"
                >
                  <FiTrash2 size={14}/>
                </button>
              </div>
...
const ItemCard = ({ item, idx, onView, onAddToCart }) => {
  ...
  return (
    <div
      className="mp-card bg-white rounded-2xl overflow-hidden border border-[color:var(--color-muted-clay)]/10 shadow-sm flex flex-col mp-fadeup hover:-translate-y-1 hover:shadow-xl transition-all duration-300"
      style={{ animationDelay: `${idx * 40}ms` }}
    >
      <div
        className="mp-card-img relative overflow-hidden bg-[color:var(--color-warm-sand)] cursor-pointer"
        style={{ height: 210 }}
        onClick={() => onView(item)}
      >
        {img
          ? <img src={img.url} alt={item.listingTitle} className="w-full h-full object-cover block transition-transform duration-300" />
          : <div className="h-full flex items-center justify-center opacity-20 text-[color:var(--color-deep-brown)]"><FiPackage size={48}/></div>
        }
        {item.isFeatured && (
        ...
        {available <= 0 && (
          <div className="absolute inset-0 bg-[color:var(--color-deep-brown)]/55 flex items-center justify-center">
            <span className="text-[13px] text-[color:var(--color-warm-sand)] font-bold tracking-wide" style={BASKERVILLE}>Out of Stock</span>
          </div>
        )}
        {available > 0 && available <= 3 && (
          <div className="absolute top-2.5 right-2.5 bg-red-500 text-white px-2.5 py-0.5 rounded-full text-[10px] font-bold" style={BASKERVILLE}>
            Only {available} left
          </div>
        )}
        <div className="mp-overlay absolute inset-0 bg-[color:var(--color-deep-brown)]/35 flex items-center justify-center">
          <span className="mp-overlay-badge bg-white/90 rounded-full px-3.5 py-1.5 text-xs font-bold text-[color:var(--color-deep-brown)] flex items-center gap-1.5" style={BASKERVILLE}>
      ...
      <div className="p-3.5 px-4 flex-1 flex flex-col gap-2">
        ...
        {item.artwork?.category && (
          <span className="inline-flex bg-[color:var(--color-sage-green)]/15 text-[color:var(--color-sage-green)] border border-[color:var(--color-sage-green)]/30 px-2.5 py-0.5 rounded-full text-[10px] font-bold w-fit" style={BASKERVILLE}>
            {item.artwork.category}
          </span>
        )}

        {item.province && (
          <div className="flex items-center gap-1 text-[11px] text-[color:var(--color-muted-clay)]" style={BASKERVILLE}>
            <FiMapPin size={10}/> {item.province}
          </div>
        )}
    ...
    <div className="flex flex-col items-center gap-3 mt-12">
      <div className="flex gap-1.5 items-center flex-wrap justify-center">
        <button onClick={() => onPage(1)} disabled={page <= 1}
          className={`${btnBase} bg-white text-[color:var(--color-deep-brown)] disabled:opacity-40 disabled:cursor-not-allowed`}
          style={BASKERVILLE}>«</button>
        ...
        {pages.map((p, i) =>
          p === '…'
            ? <span key={`e${i}`} className="text-[color:var(--color-muted-clay)] px-1 self-center" style={BASKERVILLE}>…</span>
            : <button key={p} onClick={() => onPage(p)}
                className={`${btnBase} ${p === page ? 'bg-[color:var(--color-muted-teal)] text-[color:var(--color-warm-sand)] border-[color:var(--color-muted-teal)]' : 'bg-white text-[color:var(--color-deep-brown)]'}`}
                style={BASKERVILLE}>{p}</button>
        )}
...
const Marketplace = () => {
    ...
    try {
      ...
      if (category !== 'All') {
        ...
        allItems = allItems.filter((item) => {
          const cat = (item.artwork?.category || item.category || '').toLowerCase().trim();
          return cat === sel;
        });
      }
      ...
      notify('Failed to load marketplace', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const addToCart = (item, qty = 1) => {
    setCart((prev) => {
      ...
      return ex
        ? prev.map((c) => c._id === item._id ? { ...c, qty: c.qty + qty } : c)
        : [...prev, { ...item, qty }];
    });
  ...
  return (
    <div className="bg-[color:var(--color-warm-sand)] min-h-screen" style={BASKERVILLE}>
      ...
      <HeroCarousel
        onCategorySelect={(cat) => { setCategory(cat); setPage(1); window.scrollTo({ top: 620, behavior: 'smooth' }); }}
        onSearch={(q) => { setSearch(q); setSearchInput(q); }}
      />
        ...
        <div className="max-w-[1400px] mx-auto px-6 flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="bg-[color:var(--color-muted-clay)] text-[color:var(--color-warm-sand)] rounded-full px-1.5 text-[11px] font-black">{cartCount}</span>
              )}
            </button>
          </div>
        </div>
      ...
      {hasFilters && (
        <div className="max-w-[1400px] mx-auto px-6 pt-3 flex gap-2 flex-wrap items-center">
          {search && (
            <span className="bg-[color:var(--color-muted-teal)]/10 text-[color:var(--color-muted-teal)] border border-[color:var(--color-muted-teal)]/25 px-3.5 py-1 rounded-full text-xs flex items-center gap-1.5" style={BASKERVILLE}>
              <FiSearch size={11}/> "{search}"
            </span>
          )}
          {category !== 'All' && (
            <span className="bg-[color:var(--color-sage-green)]/15 text-[color:var(--color-sage-green)] border border-[color:var(--color-sage-green)]/30 px-3.5 py-1 rounded-full text-xs" style={BASKERVILLE}>
              {category}
            </span>
          )}
          {province !== 'All' && (
            <span className="bg-[color:var(--color-muted-clay)]/10 text-[color:var(--color-muted-clay)] border border-[color:var(--color-muted-clay)]/25 px-3.5 py-1 rounded-full text-xs flex items-center gap-1.5" style={BASKERVILLE}>
              <FiMapPin size={11}/> {province}
            </span>
          )}
          {maxPrice < 500000 && (
            <span className="bg-[color:var(--color-dusty-rose)]/15 text-[color:var(--color-dusty-rose)] border border-[color:var(--color-dusty-rose)]/30 px-3.5 py-1 rounded-full text-xs" style={BASKERVILLE}>
              Max: {fmt(maxPrice)}
            </span>
          )}
          <button
            onClick={clearFilters}
            className="flex items-center gap-1.5 bg-transparent border border-red-300/40 rounded-full px-3.5 py-1 cursor-pointer text-xs text-red-500 hover:bg-red-50 transition-colors"
            style={BASKERVILLE}
          >
      ...
      {/* main layout*/}
      <div
        className="max-w-[1400px] mx-auto px-6 py-6 pb-12 grid items-start transition-all duration-300"
      ...
      >
        ...
        <div className={`overflow-hidden transition-opacity duration-300 ${sidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
          <div className="bg-white rounded-[18px] p-5 border border-[color:var(--color-muted-clay)]/10 shadow-sm sticky top-[72px]">
            ...
            <div className="mb-5">
              <label className="block text-[10px] font-bold text-[color:var(--color-muted-teal)] tracking-[.14em] uppercase mb-2" style={BASKERVILLE}>
                Category
              </label>
            ...
            <div className="mb-5">
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-xl border border-[color:var(--color-muted-clay)]/25 text-xs text-[color:var(--color-soft-charcoal)] bg-white cursor-pointer outline-none"
                style={BASKERVILLE}
              >
                ...
                {items.map((item, idx) =>
                  viewMode === 'grid'
                    ...
                    : (
                      <div
                        key={item._id}
                        className="mp-card bg-white rounded-2xl border border-[color:var(--color-muted-clay)]/10 shadow-sm flex overflow-hidden mp-fadeup hover:shadow-lg transition-all duration-200"
                      ...
                      >
                        ...
                        <div className="flex-1 p-3.5 px-4 flex items-center gap-4">
                          <div className="flex-1">
                            ...
                            {item.artwork?.category && (
                              <span className="bg-[color:var(--color-sage-green)]/15 text-[color:var(--color-sage-green)] border border-[color:var(--color-sage-green)]/30 px-2.5 py-0.5 rounded-full text-[10px] font-bold" style={BASKERVILLE}>
                                {item.artwork.category}
                              </span>
                            )}
                          </div>
